...
import { SceneUpdate } from "@foxglove/schemas/schemas/typescript/SceneUpdate";
import {
  LayoutActions,
  MessageEvent,
      ...
      >
        ...
        <Paper square={false} elevation={4} style={{ display: "flex", flexDirection: "column" }}>
          ...
          <IconButton
            ...
            title={props.drawingActive ? "Cancel drawing" : "Drawing Cuboid"}
 ...
 */
export function ThreeDeeRender2({ context }: { context: PanelExtensionContext }): JSX.Element {
  ...
  useEffect(() => {
    ...
    for (const message of messages) {
      if (message.topic === "/markers/annotations") {
        const messageEnvent = {
          topic: "/markers/new",
          schemaName: message.schemaName.toString(),
          receiveTime: message.receiveTime as Time,
          publishTime: message.publishTime as Time,
          message: message.message as DeepPartial<SceneUpdate>,
          sizeInBytes: message.sizeInBytes,
        };

        const maybeHasMarkers = messageEnvent.message;
        ...
        if (maybeHasMarkers.entities) {
          if (maybeHasMarkers.entities[0]?.cubes) {
            if (maybeHasMarkers.entities[0].cubes[0]?.pose?.position != undefined) {
              maybeHasMarkers.entities[0].cubes[0].pose.position = { x: 410, y: 1164, z: 1.6 };
              maybeHasMarkers.entities = [maybeHasMarkers.entities[0]];
            }
          }
        }
        // eslint-disable-next-line no-restricted-syntax
        ...
        // eslint-disable-next-line no-restricted-syntax
        console.log(messageEnvent.message.entities);
        renderer.addMessageEvent(messageEnvent);
      }
    }
  ...
  useEffect(() => {
    const onStart = () => setDrawingActive(true);
    const onEnd = () => setDrawingActive(false);
    renderer?.drawCuboidTool.addEventListener("foxglove.cuboid-start", onStart);
    renderer?.drawCuboidTool.addEventListener("foxglove.cuboid-end", onEnd);
    return () => {
      renderer?.drawCuboidTool.removeEventListener("foxglove.cuboid-start", onStart);
      renderer?.drawCuboidTool.removeEventListener("foxglove.cuboid-end", onEnd);
    };
  }, [renderer?.drawCuboidTool]);

  const onClickDrawing = useCallback(() => {
    if (drawingActive) {
      renderer?.drawCuboidTool.stopDrawing();
    } else {
      renderer?.drawCuboidTool.startDrawing();
